...
class TestGPIOServoInterface:
    ...
    def test_gpio_pin_configuration(self, mock_pigpio, test_config):
        ...
        # Assert
        assert mode_result == 0, f"Configuração do GPIO {gpio_pin} deve ser bem-sucedida"
        assert actual_mode == mock_pigpio.OUTPUT, f"GPIO {gpio_pin} deve estar configurado como OUTPUT"
    ...
    def test_pwm_frequency_setting(self, mock_pigpio, test_config):
        ...
        # Assert
        assert freq_result >= 0, "Configuração de frequência deve ser bem-sucedida"
        assert actual_frequency == servo_frequency, f"Frequência deve ser {servo_frequency}Hz"
    ...
    def test_servo_pulsewidth_range_validation(self, mock_pigpio, test_config):
        ...
        for pulsewidth in test_pulsewidths:
            ...
            # Assert
            assert result == 0, f"Pulsewidth {pulsewidth}µs deve ser aplicada com sucesso"
            assert actual_pulsewidth == pulsewidth, f"Pulsewidth atual deve ser {pulsewidth}µs"
    ...
    def test_servo_pulsewidth_limits(self, mock_pigpio, test_config):
        ...
        for pulsewidth in invalid_pulsewidths:
            ...
            if pulsewidth == 0:
                assert result == 0, "Pulsewidth 0 deve desligar servo (válido)"
            else:
                # Para valores fora da faixa, pigpio pode retornar erro ou clampar
                assert isinstance(result, int), "Resultado deve ser inteiro"
...
class TestVESCUSBInterface:
    ...
    def test_vesc_device_presence_simulation(self):
        ...
        # Assert
        assert device_exists, f"Dispositivo VESC deve existir em {expected_device}"
        assert device_readable, "Dispositivo deve ser acessível pelo usuário"
    ...
    def test_vesc_serial_parameters_validation(self):
        ...
        assert serial_config['baudrate'] == 115200, "Baudrate deve ser 115200"
        assert serial_config['timeout'] <= 0.1, "Timeout deve ser ≤100ms"
        assert serial_config['bytesize'] == 8, "Deve usar 8 bits de dados"
        assert serial_config['parity'] == 'N', "Deve usar sem paridade"
        assert serial_config['stopbits'] == 1, "Deve usar 1 stop bit"
    ...
    def test_vesc_speed_to_duty_conversion(self):
        ...
        for speed in test_speeds:
            ...
            assert -0.5 <= duty_cycle <= 0.5, \
                f"Duty cycle {duty_cycle} para velocidade {speed}m/s deve estar na faixa válida"
...
class TestServoCalibrationDiscovered:
    ...
    def test_servo_center_calibration(self, mock_pigpio, test_config):
        ...
        assert actual_pulsewidth == center_pulsewidth, f"Centro deve ser {center_pulsewidth}µs"

    def test_servo_left_right_calibration(self, mock_pigpio, test_config):
        """Testa calibração esquerda/direita do servo."""
        # Arrange - Calibração descoberta
        calibration_points = [
            (test_config.min_steering_angle, test_config.servo_min_pulse_width),  # Esquerda
            (test_config.max_steering_angle, test_config.servo_max_pulse_width),  # Direita
        ]
        gpio_pin = test_config.servo_gpio_pin
        ...
        for angle, expected_pulsewidth in calibration_points:
            ...
            # Assert
            assert result == 0, f"Comando para ângulo {angle} deve ser bem-sucedido"
    ...
    def test_servo_intermediate_positions(self, mock_pigpio, test_config):
        ...
        for angle, expected_pulsewidth in test_cases:
            ...
            assert abs(actual_pulsewidth - expected_pulsewidth) <= 5, \
                f"Pulsewidth para {angle} rad deve estar próximo de {expected_pulsewidth}µs"
...
class TestHardwareLimitsAndSafety:
    ...
    def test_servo_angle_physical_limits(self, mock_pigpio, test_config):
        ...
        max_pulsewidth = int(test_config.servo_min_pulse_width + normalized_angle * pulse_range)
        
        result_max = mock_pigpio.set_servo_pulsewidth(gpio_pin, max_pulsewidth)
        
        # Act - Comando para limite mínimo
        normalized_angle_min = (min_safe_angle - min_safe_angle) / angle_range
        min_pulsewidth = int(test_config.servo_min_pulse_width + normalized_angle_min * pulse_range)
        
        result_min = mock_pigpio.set_servo_pulsewidth(gpio_pin, min_pulsewidth)
        
        # Assert
        assert result_max == 0, "Comando para ângulo máximo seguro deve funcionar"
        assert result_min == 0, "Comando para ângulo mínimo seguro deve funcionar"
        assert max_pulsewidth <= test_config.servo_max_pulse_width, "Não deve exceder máximo físico"
        assert min_pulsewidth >= test_config.servo_min_pulse_width, "Não deve exceder mínimo físico"
    ...
    def test_vesc_current_protection_simulation(self):
        ...
        for current in test_currents:
            ...
            assert motor_current_safe, f"Corrente motor {current}A deve estar abaixo do limite"
            if current <= max_battery_current:
                assert battery_current_safe, f"Corrente bateria {current}A deve estar abaixo do limite"
    ...
    def test_emergency_stop_hardware_response(self, mock_pigpio, test_config):
        ...
        # Arrange
        gpio_pin = test_config.servo_gpio_pin
        emergency_angle = 0.0  # Posição central segura
        ...
        # Assert
        assert result == 0, "Comando de emergência deve ser executado imediatamente"
        assert actual_pulsewidth == emergency_pulsewidth, "Servo deve ir para posição segura"
    ...
    def test_power_supply_voltage_validation(self):
        ...
        for component, voltage in expected_voltages.items():
            assert voltage > 0, f"Tensão de {component} deve ser positiva"
            assert voltage <= 24.0, f"Tensão de {component} deve estar abaixo de 24V (segurança)"
...
class TestROS2HardwareIntegration:
    ...
    def test_ros2_servo_topic_to_hardware(self, mock_pigpio, test_config):
        ...
        # Limitação de ângulo
        clamped_angle = max(min(ackermann_steering_angle, test_config.max_steering_angle), 
        ...
        # Assert
        assert result == 0, "Comando ROS2 deve ser aplicado ao hardware"
        assert 1000 <= pulse_width <= 2000, "Comando deve estar na faixa válida"
        assert pulse_width > 1500, "Ângulo positivo deve resultar em PWM > centro"
    ...
    def test_vesc_ros2_integration_simulation(self):
        ...
        # Assert
        assert -0.5 <= duty_cycle <= 0.5, "Duty cycle deve estar na faixa VESC"
        assert duty_cycle > 0, "Velocidade positiva deve resultar em duty cycle positivo"
    ...
    def test_hardware_feedback_to_ros2(self):
        ...
        # Assert
        assert distance_traveled > 0, "Distância calculada deve ser positiva"
        assert distance_traveled < 10.0, "Distância deve ser razoável"
...
class TestSystemCleanupAndShutdown:
    ...
    def test_servo_cleanup_on_shutdown(self, mock_pigpio, test_config):
        ...
        center_result = mock_pigpio.set_servo_pulsewidth(gpio_pin, safe_position)
        ...
        # Assert
        assert center_result == 0, "Deve conseguir ir para posição segura"
        assert off_result == 0, "Deve conseguir desligar PWM"
        assert cleanup_successful, "Limpeza deve ser bem-sucedida"
    ...
    def test_vesc_shutdown_procedure(self):
        ...
        # Assert
        assert motor_stopped, "Motor deve estar parado no shutdown"
        assert current_zero, "Corrente deve ser zero no shutdown"
